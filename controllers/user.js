...
exports.verifyEmployeeOTP = async (req, res) => {
    ...
    const { email, password, full_name, address, phone,designation,department,otp,companyId } = req.body;
    // Hash the password for security
    const hashedPassword = await bcrypt.hash(password, 10);
...
exports.companyAddEmployee = async (req, res) => {
    try {
        ...
        const company = await model.company.findOne({ where: { userId: authUser.id } });
        if (!company) {
            return res.status(404).json({
                success: false,
        ...
        const { email, password, firstName,lastName, address, phone, designation, department, cnic } = req.body;

        // Check if required fields are provided
        if (!email || !password || !firstName || !lastName || !address || !phone || !cnic) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields: email, password, full_name, address, phone, cnic'
            });
        }
        ...
        if (req.file) {
            ...
            profileImagePath = path.relative(path.join(__dirname, '../public'), filePath); // Excludes `public/` and saves the relative path
        } 
...
exports.addProjectForCompany = async (req, res) => {
    const { name, description, budget, startDate, endDate, clientName, clientEmail } = req.body;
    ...
    // Input validation
    if (!name || !description || !budget || !startDate || !endDate || !clientName || !clientEmail) {
        return res.status(400).json({
            success: false,
    ...
    try {
        ...
        const requirementsFilePath = path.join(__dirname, '../uploads', companyName, 'projects', req.file.filename);
        ...
        const requirementsRelativePath = path.join('/', companyName, 'projects', req.file.filename); // This is the relative path
...
exports.addOrUpdateProjectEmployee = async (req, res) => {
    const authUser = req.user;
    const { projectId, employeeId, roleInProject, hoursAllocated, status } = req.body;
    ...
    if (!projectId || !employeeId || !roleInProject || !hoursAllocated) {
        return res.status(400).json({
            success: false,
            message: 'Project ID, Employee ID, Role in Project, and Hours Allocated are required.',
        });
    }
...
exports.addTaskToProject = async (req, res) => {
    ...
    try {
        ...
        // Step 1: Check if the project exists and belongs to the authenticated user's company
        ...
        // Step 2: Check if a task with the same name already exists for the project
        ...
        // Step 3: Validate required fields for the task
        if (!name) {
            return res.status(400).json({
                success: false,
        ...
        // Step 4: Handle file upload (if any)
        let detailsFilePath = null;
        if (req.file) {
            // Adjust the file path to omit the 'public' directory
            detailsFilePath = req.file.path.replace(path.join(__dirname, '../public'), '');
        }

        // Step 5: Create the task and associate it with the project
        const newTask = await model.task.create({
            ...
            endDate,
            projectId,
            detailsFile: detailsFilePath // Save the file path in the task if the file was uploaded
        });
    ...
    try {
        ...
        const task = await model.task.findOne({
            ...
            include: {
                ...
                include: {
                    model: model.company,
                    where: { userId: user.id } // Ensure the task belongs to the authenticated user's company
                }
            }
        });
        ...
        const completedModules = modules.filter(module => module.completionPercentage === "100").length;
        ...
        res.status(200).json({
            ...
            data: {
                task,
                modules,
                taskProgressPercentage: Math.round(taskProgressPercentage) // Round to nearest integer
            }
        });
...
exports.assignOrUpdateEmployeeToTask = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({
            success: false,
            message: 'An error occurred while assigning or updating the employee task.',
            error: error.message
        });
    }
};
...
exports.getOneProject = async (req, res) => {
    ...
    try {
        ...
        const project = await model.project.findOne({
            ...
            include: {
                model: model.company,
                where: { userId: user.id } // Ensure the project belongs to the authenticated user's company
            }
        });
        ...
        const tasks = await model.task.findAll({
            ...
            include: {
                model: model.modules,
                attributes: ['progress'] // Only fetch the progress field for modules
            }
        });
        ...
        tasks.forEach(task => {
            if (task.modules.length > 0) {
                ...
                const completedModules = task.modules.filter(module => module.progress === 100).length;
        ...
        res.status(200).json({
            ...
            data: {
                project,
                tasks,
                projectProgressPercentage: Math.round(projectProgressPercentage) // Round to nearest integer
            }
        });
...
exports.addOrUpdateModuleForTask = async (req, res) => {
    const {  moduleId, name, description, status, startDate, endDate } = req.body;
